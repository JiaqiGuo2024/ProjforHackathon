...
export const Dashboard: React.FC = () => {
  ...
  const { documents, notes, ideas, events, sidebarOpen, addDocument } = useWorkspaceStore();
  ...
  const handlePdfUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && file.type === 'application/pdf' && user) {
      // Create a new document entry
      const newDocument: Document = {
        id: nanoid(),
        title: file.name.replace('.pdf', ''),
        type: 'pdf',
        url: URL.createObjectURL(file), // In a real app, upload to storage service
        owner_id: user.id,
                  ...
                  if (action.onClick) {
                    return (
                      <button
                        ...
                        className={`${action.color} text-white p-6 rounded-lg transition-colors group text-left`}
                      >
