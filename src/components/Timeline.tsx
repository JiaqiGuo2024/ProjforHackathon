...
import { Clock, Users, MapPin, Flag, CheckCircle } from 'lucide-react';
...
export const Timeline: React.FC<TimelineProps> = ({ events, selectedDate }) => {
  const { projects, getProjectMilestones } = useProjectStore();

  const dayEvents = events
    .filter(event => isSameDay(new Date(event.start_time), selectedDate))
    .sort((a, b) => new Date(a.start_time).getTime() - new Date(b.start_time).getTime());

  // Get project milestones for the selected date
  const dayMilestones = projects.flatMap(project => {
    const milestones = getProjectMilestones(project.id);
    return milestones
      .filter(milestone => isSameDay(new Date(milestone.due_date), selectedDate))
    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 h-full">
      ...
      <div className="p-4 border-b border-gray-200">
        ...
        <p className="text-sm text-gray-500 mt-1">
          {dayEvents.length} event{dayEvents.length !== 1 ? 's' : ''} • {dayMilestones.length} milestone{dayMilestones.length !== 1 ? 's' : ''}
        </p>
      </div>
      ...
      {dayMilestones.length > 0 && (
        <div className="p-4 border-b border-gray-200 bg-blue-50">
          ...
          <div className="space-y-2">
            {dayMilestones.map(({ project, ...milestone }) => (
              <div key={milestone.id} className="flex items-center space-x-3 p-2 bg-white rounded border border-blue-200">
                <div className={`w-4 h-4 rounded-full flex items-center justify-center ${
                  milestone.completed ? 'bg-green-600 text-white' : 'bg-blue-600 text-white'
                }`}>
                  {milestone.completed ? (
      ...
      {/* Quick Stats */}
      {(dayEvents.length > 0 || dayMilestones.length > 0) && (
        <div className="p-4 border-t border-gray-200 bg-gray-50">
          <div className="grid grid-cols-3 gap-4 text-center">
            ...
            <div>
              <div className="text-lg font-semibold text-gray-900">{dayMilestones.length}</div>
